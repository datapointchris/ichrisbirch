...
- **Pre-commit hooks** run automatically; the set is generated from `~/tools/forge/pre-commit/toolchain.yml` (see `~/dev/standards/ci.md`). Repo-specific detail: Vue hooks only trigger on `frontend/**/*.{vue,ts,tsx,js,jsx}`.
...
- **NEVER modify `sys.path`** — use standard imports. Use `find_project_root()` from `ichrisbirch.util` instead of `Path(__file__).parent.parent.parent`. (The general rule is in `~/dev/standards/python.md`; `find_project_root()` is the ichrisbirch-specific affordance.)
- **Schema conventions** — `Text` columns, lookup tables not PG enums, PK strategy, nullability: see `~/dev/standards/data.md`. ichrisbirch is the fleet's one sanctioned Postgres app.
