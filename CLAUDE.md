...
**Refresh is serialized machine-wide** (⚠️ MANDATORY): Authelia rotates the refresh token on every use and revokes the entire grant when a consumed one is presented again, which it reads as a stolen token. Two `icb` processes refreshing from the same stored token therefore cost an interactive re-login rather than a retry — one rotates, the other replays the token that rotation consumed, and Authelia drops the pair. Several processes run at once in ordinary use, because `doit` resolves each pursuit's evidence in its own `icb` invocation. `internal/auth` holds an exclusive `flock` on `$XDG_STATE_HOME/icb/<client_id>.refresh.lock` for the whole refresh and re-reads the keychain after taking it, so a process that waited uses the winner's token instead of replaying its own. The serialization has to span processes; a mutex is not enough, and `TestTokenSource_ConcurrentProcessesRefreshOnce` fails without the lock.
...
- **A time column is one of three kinds, and the kind decides the type** (⚠️ MANDATORY): pick before writing the column, because all three are spelled `datetime` in Python and only one of them should be.
  1. **A moment that happened** — a completion, a login, a row appearing. `DateTime(timezone=True)`, and the value is always `datetime.now(UTC)` or `server_default=func.now()`. Never `datetime.now()`: naive is interpreted using Postgres's session `TimeZone`, so it is correct only while every container happens to run UTC.
  ...
  3. **A future local wall clock** — an event at a venue. Naive `DateTime(timezone=False)` plus a `Text` column holding the IANA zone name. Storing an instant is wrong here, because a tzdata change moves it and because 19:00 at the venue is 19:00 for every reader.
...
- **An offset is not a timezone.** `-05:00` cannot say what the local time will be on a future date, so a stored zone is an IANA name (`America/New_York`) and is validated as one.
