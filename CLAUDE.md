...
- **Which day an instant falls on is read in the user's zone, never in UTC or the container's.** The user's zone is the `timezone` preference, which the web app fills from the browser for a user with none; `User.calendar_zone` answers UTC only before that.
  - **API**: a bare-day bound on a timestamp column is read in `RequestZone` (`api/request_zone.py`) — the `timezone` query parameter when the caller sends one, else `User.calendar_zone`. `apply_date_bounds` raises on a bare day against a timestamp column with no zone.
