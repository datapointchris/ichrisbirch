...
class BookCreateForm(FlaskForm):
    ...
    purchase_date = DateTimeField('Purchase Date', validators=[Optional()], render_kw={"type": "date"})
    purchase_price = FloatField('Purchase Price', validators=[Optional()])
    sell_date = DateTimeField('Sell Date', validators=[Optional()], render_kw={"type": "date"})
    sell_price = FloatField('Sell Price', validators=[Optional()])
    read_start_date = DateTimeField('Read Start Date', validators=[Optional()], render_kw={"type": "date"})
    read_finish_date = DateTimeField('Read Finish Date', validators=[Optional()], render_kw={"type": "date"})
