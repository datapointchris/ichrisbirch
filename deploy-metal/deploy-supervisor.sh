...
CONFIG_FILES=("api.conf" "app.conf" "scheduler.conf")
LOG_FILES=("supervisord.log" "$PROJECT_NAME-api.log" "$PROJECT_NAME-app.log" "$PROJECT_NAME-scheduler.log")
...
if [[ $SUPERVISORD_UPDATED ]]; then
  ...
  echo restarting supervisor processes only
