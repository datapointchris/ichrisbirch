...
def test_create(test_api):
    crud_tests.test_create(test_api, verify_attr='title')
...
def test_lifecycle(test_api):
    crud_tests.test_lifecycle(test_api, verify_attr='title')


def test_read_current(test_api):
    response = test_api.get(f'{ENDPOINT}current/')
    assert response.status_code == status.HTTP_200_OK, show_status_and_response(response)
    assert response.json() is not None


def test_archive(test_api):
    first_id = crud_tests.item_id_by_position(test_api, position=1)
    response = test_api.patch(f'{ENDPOINT}{first_id}/', json={'is_archived': True})
    assert response.status_code == status.HTTP_200_OK, show_status_and_response(response)


def test_unarchive(test_api):
    first_id = crud_tests.item_id_by_position(test_api, position=1)
    response = test_api.patch(f'{ENDPOINT}{first_id}/', json={'is_archived': False})
    assert response.status_code == status.HTTP_200_OK, show_status_and_response(response)


def test_favorite(test_api):
    first_id = crud_tests.item_id_by_position(test_api, position=1)
    response = test_api.patch(f'{ENDPOINT}{first_id}/', json={'is_favorite': True})
    assert response.status_code == status.HTTP_200_OK, show_status_and_response(response)


def test_unfavorite(test_api):
    first_id = crud_tests.item_id_by_position(test_api, position=1)
    response = test_api.patch(f'{ENDPOINT}{first_id}/', json={'is_favorite': False})
    assert response.status_code == status.HTTP_200_OK, show_status_and_response(response)


def test_read(test_api):
    first_id = crud_tests.item_id_by_position(test_api, position=1)
    article = test_api.get(f'{ENDPOINT}{first_id}/').json()
