...
**`deploy-containers/traefik/dynamic/prod/routing.yml`** (git-tracked) defines all routers (rules, middleware, priorities). **`services.yml`** (gitignored, generated by deploy script) defines services pointing to the active color's containers. Traefik merges both files from the directory provider. Switching traffic means regenerating `services.yml`:

```yaml
# services.yml — generated by deploy script
http:
  services:
    api:
      loadBalancer:
        servers:
          - url: "http://icb-green-api:8000"  # ← blue or green
    vue:
      loadBalancer:
        servers:
...
4. Regenerates `services.yml` to point to the previous color
...
| `ops/icbops` | CLI with blue/green-aware commands |
| `ops/health-check.sh` | Health check script (reads active color from state file) |
