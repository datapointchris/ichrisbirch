...
class MarkdownCodeReference:
    ...
    def __init__(
        ...
        self.lines = lines  # (start_line, end_line) tuple for line-based extraction
...
class CodeSyncTool:
    ...
    def __init__(self, base_path: str | Path | None = None):
        ...
        # Pattern for element-based code blocks: ```lang file=path element=name
        self._code_block_element_pattern = re.compile(
            r'```([\w-]*)\s+'  # Language identifier
            r'(?:code|file)=([^\s]+)\s+'  # File path
            r'(?:element|name)=([^\s]+)'  # Element name
            r'(?:\s+([^\n]*))?'  # Optional attributes
            r'\n'  # Newline after metadata
            r'(.*?)'  # Code content (lazy match)
            r'```',  # Closing code block
            re.DOTALL,
        )

        # Pattern for line-based code blocks: ```lang file=path lines=X-Y
        self._code_block_lines_pattern = re.compile(
            r'```([\w-]*)\s+'  # Language identifier
            r'(?:code|file)=([^\s]+)\s+'  # File path
            r'lines=(\d+)-(\d+)'  # Line range
            r'(?:\s+([^\n]*))?'  # Optional attributes
            r'\n'  # Newline after metadata
            r'(.*?)'  # Code content (lazy match)
            r'```',  # Closing code block
        ...
        # Inline element reference: [code:file=path.py:element=name]
        self._inline_code_pattern = re.compile(
            r'\[code:'  # Opening tag
            r'(?:code|file)=([^\s:]+):'  # File path
            r'(?:element|name)=([^\s:\]]+)'  # Element name
            r'(?::([^\]]*))?\]'  # Optional attributes and closing tag
        )

        # Inline line reference: [code:file=path.py:lines=X-Y]
    ...
    def extract_code_references(self, markdown_content: str) -> list[MarkdownCodeReference]:
        ...
        # Extract element-based code blocks
        ...
        # Extract line-based code blocks
        ...
        # Extract inline element references
        for match in self._inline_code_pattern.finditer(markdown_content):
            ...
            ref = MarkdownCodeReference(
                file_path=file_path,
                element_name=element_name,
                language='',  # Inline references don't specify language
                start_pos=match.start(),
                end_pos=match.end(),
                original_text=match.group(0),
                attrs=attrs,
        ...
        # Extract inline line references
    ...
    def resolve_file_path(self, file_path: str, markdown_file: Path | None = None) -> Path:
        ...
        # Not found, return base path + file path anyway
    ...
    def sync_markdown_file(self, markdown_file: str | Path) -> bool:
        ...
        try:
            ...
            for ref in sorted(references, key=lambda r: r.start_pos):
                ...
                # Get the code content based on reference type
                if ref.is_line_based and ref.lines is not None:
                    ...
                    # Element-based extraction via AST
                ...
                # Update content with consideration for any previous offsets
    ...
    def _create_replacement(self, ref: MarkdownCodeReference, code_content: str) -> str:
        ...
        if ref.original_text.startswith('```'):
            ...
            # Inline format - preserve the original (inline refs just mark the location)
            attrs_str = ':'.join(f'{k}={v}' for k, v in ref.attrs.items())
            if attrs_str:
                attrs_str = ':' + attrs_str
...
""",
    ...
    if args.check:
        ...
        # Normal mode: sync and update files
...
def _run_check_mode(syncer: CodeSyncTool, docs_dir: str) -> int:
    ...
    for md_file in markdown_files:
        ...
        # Create a temporary copy to sync
    ...
    if out_of_sync:
        ...
        logger.error("\nRun 'python -m tools.code_sync docs --base-path .' to sync them.")
        return 1
    else:
