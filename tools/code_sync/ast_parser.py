...
class ASTParser:
    ...
    def _extract_elements(self, tree: ast.AST, source: str, parent: str | None = None) -> None:
        ...
        for node in ast.iter_child_nodes(tree):
            if isinstance(node, ast.FunctionDef | ast.AsyncFunctionDef):
                ...
                # Extract class (including decorators)
                name = node.name
                start_line = self._find_start_line_with_decorators(node)
                end_line = self._find_end_line(node, source)
                ...
                if parent is None:  # Only extract top-level for now
    ...
    def _find_end_line(self, node: ast.AST, source: str) -> int:
        ...
        # Get the last child node's line number
...
class TreeSitterParser:
    ...
    def __init__(self):
        ...
        try:
            # Import Tree-sitter only if available
    ...
    def parse_file(self, file_path: str | Path) -> dict[str, CodeElement]:
        ...
        # Implementation for Tree-sitter parsing would go here
        # This is a placeholder and would need to be implemented based on the languages you want to support
...
class FallbackParser:
    ...
    def parse_file(self, file_path: str | Path, pattern: str | None = None) -> dict[str, CodeElement]:
        ...
        try:
            ...
            # Default patterns for common languages if none provided
            if pattern is None:
            ...
            for name, regex in patterns.items():
                for match in regex.finditer(source):
                    if name == '_custom_':
                        # If we're using a custom pattern, use the first capture group as the name
                        element_name = match.group(1) if match.groups() else 'unnamed'
                    else:
                        element_name = name

                    # Calculate line numbers from source up to match position
    ...
    def _get_default_patterns(self, file_extension: str) -> dict[str, re.Pattern]:
        ...
        if file_extension == '.js' or file_extension == '.ts':
            # JavaScript/TypeScript function pattern - more permissive to handle various formats
            patterns['simpleFunction'] = re.compile(r'function\s+simpleFunction\s*\([^{]*\)\s*\{[\s\S]*?\}', re.DOTALL)
            # JavaScript/TypeScript class pattern - more permissive
        ...
        elif file_extension == '.c' or file_extension == '.cpp' or file_extension == '.h':
            # C/C++ function pattern (simplified)
...
class CodeParser:
    ...
    def parse_file(self, file_path: str | Path) -> dict[str, CodeElement]:
        ...
        if file_path.suffix.lower() == '.py':
            ...
            if result:  # If Tree-sitter found anything
                return result

        # Fallback to regex if other parsers aren't available or didn't find anything
    ...
    def find_code_element(self, file_path: str | Path, element_name: str) -> CodeElement | None:
        ...
        # If not found, try a more flexible matching
        # This allows for matching just the method name without the class prefix
