...
watch_run() {
  ...
  while true; do
    local current_info
    current_info=$(gh run list --workflow="$WORKFLOW_NAME" --limit=1 --json databaseId,status,conclusion --jq '.[0]')
    local current_id
    current_id=$(echo "$current_info" | jq -r '.databaseId')
    local current_status
    current_status=$(echo "$current_info" | jq -r '.status')
    local current_conclusion
    current_conclusion=$(echo "$current_info" | jq -r '.conclusion // "pending"')
