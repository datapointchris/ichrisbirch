...
generate_services_file() {
  ...
  # The YAML itself is written by scripts/generate-services-yml.sh, which the
  # CLI's rollback path also calls.
  local color="$1"
  local services_file="${INSTALL_DIR}/deploy-containers/traefik/dynamic/prod/services.yml"

  "${INSTALL_DIR}/scripts/generate-services-yml.sh" "$color" "$services_file" 'deploy script'
}
