...
pull_new_images() {
  ...
  local pull_log
  pull_log="${LOG_DIR}/pull-$(date +%Y%m%d-%H%M%S).log"
...
generate_services_file() {
  # Generate services.yml pointing to the given color.
  # Routers are in routing.yml (git-tracked); services are here (deploy-time state).
  # Traefik merges both files from the directory provider.
  local color="$1"
  local services_file="${INSTALL_DIR}/deploy-containers/traefik/dynamic/prod/services.yml"
...
switch_traffic() {
  ...
  generate_services_file "$DEPLOY_COLOR"
