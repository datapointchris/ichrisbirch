...
def test_delete_task(postgres_testdb_in_docker, insert_test_data, test_app, task_id):
    task = test_app.get(endpoint(ENDPOINT, task_id))
    response = test_app.delete(endpoint(ENDPOINT, task_id))
    deleted = test_app.get(endpoint(ENDPOINT, task_id))
    assert response.status_code == 200
...
def test_task_lifecycle(postgres_testdb_in_docker, insert_test_data, test_app, test_task):
    ...
    response_task = test_app.get(endpoint(ENDPOINT, task_id)).json()
    deleted_task = test_app.delete(endpoint(ENDPOINT, task_id)).json()
    missing_task = test_app.get(endpoint(ENDPOINT, task_id))
